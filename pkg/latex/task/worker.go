...
func init() {
	...
	tasks[LatexHistoryHandleJobKey] = LatexHistoryHandleJob
}
...
func Setup() {
	...
	brokerDB := viper.GetString("redis.latexTaskBrokerDB")
	resultBackendDB := viper.GetString("redis.latexTaskResultBackendDB")

	queue = viper.GetString("latexTask.queue")              // 队列名
	concurrency := viper.GetInt("latexTask.concurrency")    // 并发数
	consumerTag := viper.GetString("latexTask.consumerTag") // 标签
